...
parser.add_argument("--port", help="set the COM port,\
     default is /dev/ttyUSB0 .", type = str)
...
read_reg[10]    =   Err_cpu
read_reg[11]    =   Reset_ALL


###############################################################################
#   Socket  :
###############################################################################
client_uart1 = socket.socket()       
host = socket.gethostname()        
user_port = 4877                         
WARNING_ResetAll_UART1    =   b'\xab'
...
filename    = 'Test_Board_Uart-' + time.strftime("%H.%M.%S") + '-' + 'log'
...
while(True):
    try: 
        ...
        client_uart1.connect((host, user_port))     
        link_down_count = 1
        break
    except OSError:
    ...
    try:
        ...
        while(True):
            ...
            if( write_occur == 0 ):
                ...
                data_tmp        =   []
                tb              =   PrettyTable()
                tb.field_names  =   ["TestBoard",' ',"time",time.strftime("%H:%M:%S"),\
                ...
                if ( sock_flag & (data_tmp[-1] != 0) ):
                    try:
                        warn_info = WARNING_ResetAll_UART1
