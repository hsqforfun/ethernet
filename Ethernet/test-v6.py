...
def check_number(Buffer):
    ...
    for i in len(Buffer):
        Buffer_number.append( bytes_to_int(Buffer[16:20]) )
    ...
    for ii in len(Buffer_number-1):
        check_buffer.append( Buffer_number[i+1] - Buffer_number[i] )
    
    for iii in len(check_buffer):
        if( check_buffer[iii] > 12 ):
            if( (check_buffer[iii+1] != None) & check_buffer[iii+1] >0 ):
                LinkDown = 1
            elif( check_buffer[iii+1] == None ):
                LinkDown = 1
            else:
                pass
        else:
