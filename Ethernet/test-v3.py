...
#                   cmd=> sar -n DEV 1 100
# 
#   Description:    modified based on test-v2
#                   aim to solve check numberID problem
#                   unfinished.....
#############################################################################
...
def send():
    ...
    while(count_send < loop_size):
        ...
        numberID_bytes = int_to_bytes( numberID , 4 )
        number = struct.pack( "4s",numberID_bytes[::-1] )
# Here, we actively inject an error packet
        if(count_send == loop_size/2):
            error_data  =  int_to_bytes( (data_init+1), 1 ) 
            data_packet =  header + pad + number + bytes(1003) + error_data
        else:
            data_packet =  header + pad + number + send_data
...
def check_raw( packet_in ):
    ...
    if( packet_in[0:6] != Src_MAC ):            # source and destination must change position
        check_bit( packet_in[0:6], Src_MAC )    # due to the send and receive direction
    else:
        pass
    ...
    if( packet_in[12:14] != Type_DEF ):
        ...
        pass
# Although above three part must match, we can not receive packet if not match.
# Here we still check them .
...
def check_bit( under_check, golden ):
    ...
    for i in range( len(under_check_bin) ):
...
# def check_num( buffer_id ):
#     golden_id   =   []
#     error_num   =   []
#     for i in range(loop_size):
#         golden_id.append( (i+1)*2 )

#     assert golden_id[-1] == (loop_size*2)

#     for ii in range( len(buffer_id) ):
#         if ( buffer_id[ii] == golden_id[ii] ):
#             pass
#         else:
